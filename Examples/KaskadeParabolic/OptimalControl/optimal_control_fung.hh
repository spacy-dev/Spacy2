...
namespace Kaskade
{
    ...
    template < class Ids, class Constraint, class CostFunctional, class AnsatzVars_,
               class TestVars_ = AnsatzVars_, RoleOfFunctional role = RoleOfFunctional::NORMAL >
    ...
    public:
        ...
        static int const ySIdx =
            boost::fusion::result_of::value_at_c< typename AnsatzVars::Variables,
                                                  Ids::state >::type::spaceIndex;
        static int const uSIdx =
            boost::fusion::result_of::value_at_c< typename AnsatzVars::Variables,
                                                  Ids::control >::type::spaceIndex;
        static int const pSIdx =
            boost::fusion::result_of::value_at_c< typename AnsatzVars::Variables,
                                                  Ids::adjoint >::type::spaceIndex;

        class DomainCache : public CacheBase< StepFunctional, DomainCache >
        {
        public:
            DomainCache( StepFunctional const& f, typename AnsatzVars::VariableSet const& vars_,
                         int )
                : f_( f ), vars( vars_ ), constraint_( f_.constraint_ ),
                  costFunctional_( f_.costFunctional_ ), update_reference_( f_.update_reference_ )
            {
            }
                ...
                const
            {
                if ( row == Ids::state )
                    return costFunctional_.template d1< Ids::state >( if_( arg.value, y ) ) +
                           sp( dp, constraint_.template d1< Ids::state >( std::make_tuple(
                                       if_( arg.value, y ), if_( arg.derivative, dy ) ) ) );
                if ( row == Ids::control )
                    return costFunctional_.template d1< Ids::control >( if_( arg.value, u ) ) -
                           sp( p, if_( arg.value, u ) );
                if ( row == Ids::adjoint )
                    return sp( if_( arg.derivative, dp ), constraint_() ) -
                           sp( if_( arg.value, p ), u );
                return 0;
            }
            ...
            Scalar d2_impl(
                ...
                VariationalArg< Scalar, dim, TestVars::template Components< col >::m > const& arg2 )
                const
            {
                if ( row == Ids::state && col == Ids::state )
                {
                    if ( role == RoleOfFunctional::TANGENTIAL )
                        return costFunctional_.template d2< Ids::state, Ids::state >(
                                   if_( arg1.value, y ), if_( arg2.value, y ) ) +
                               sp( dp, constraint_.template d2< Ids::state, Ids::state >(
                                           std::make_tuple( if_( arg1.value, y ),
                                                            if_( arg1.derivative, dy ) ),
                                           std::make_tuple( if_( arg2.value, y ),
                                                            if_( arg2.derivative, dy ) ) ) );
                    else
                        return costFunctional_.template d2< Ids::state, Ids::state >(
                                   if_( arg1.value, y ), if_( arg2.value, y ) ) +
                ...
                if ( row == Ids::control && col == Ids::control )
                    return costFunctional_.template d2< Ids::control, Ids::control >( arg1.value,
                                                                                      arg2.value );
                ...
                if ( row == Ids::state && col == Ids::adjoint )
                    return sp( if_( arg2.derivative, dp ),
                               constraint_.template d1< Ids::state >( std::make_tuple(
                                   if_( arg1.value, y ), if_( arg1.derivative, dy ) ) ) );
                if ( row == Ids::adjoint && col == Ids::state )
                    return sp( if_( arg1.derivative, dp ),
                               constraint_.template d1< Ids::state >( std::make_tuple(
                                   if_( arg2.value, y ), if_( arg2.derivative, dy ) ) ) );
                return 0;
            }
        ...
        using BoundaryCache =
            Optimization::HomogeneousDirichletBoundary< StepFunctional, Ids::state, Ids::adjoint >;

        explicit StepFunctional(
            Constraint constraint, CostFunctional costFunctional,
    ...
    template < class Ids, class Constraint, class CostFunctional, class AnsatzVars,
               class TestVars = AnsatzVars >
    using NormalStepFunctional = StepFunctional< Ids, Constraint, CostFunctional, AnsatzVars,
                                                 TestVars, RoleOfFunctional::NORMAL >;

    template < class Ids, class Constraint, class CostFunctional, class AnsatzVars,
               class TestVars = AnsatzVars >
    using TangentialStepFunctional = StepFunctional< Ids, Constraint, CostFunctional, AnsatzVars,
                                                     TestVars, RoleOfFunctional::TANGENTIAL >;
}
