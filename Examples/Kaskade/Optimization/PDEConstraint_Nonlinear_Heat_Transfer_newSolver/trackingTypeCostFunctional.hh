...
namespace Kaskade
{
    ...
    public:
            ...
            Evaluators const& evaluators )
        {
            y = y_;
            u = u_;
            y_ref = boost::fusion::at_c< stateId >( y_ref_fse.data )
                        .value( boost::fusion::at_c< stateSpaceId >( evaluators ) );
        }
        ...
        {
            if ( row == stateId )
                return beta * ( ( y - y_ref ) * d.d0() ) *
                       ( if_( arg.value, y ) * d.d0() ); //( (y - y_ref) * d.d0() ) * (
                                                         //if_(arg.value,y) * d.d0() + y * d.d1(arg)
                                                         //);
            if ( row == controlId )
                return alpha * ( u * if_( arg.value, u ) );
            return 0;
        }
            ...
            const
        {
            if ( row == stateId && col == stateId )
                return beta * ( if_( arg1.value, y ) * d.d0() ) * ( if_( arg2.value, y ) * d.d0() );
                    /*( if_(arg1.value,y) * d.d0() + y * d.d1(arg1) ) * ( if_(arg2.value,y) * d.d0() + y * d.d1(arg2) )
          + ( (y - y_ref) * d.d0() ) * ( if_(arg1.value,y) * d.d1(arg2) + if_(arg2.value,y) * d.d1(arg1) );*/ // if_(arg1.value,y) * if_(arg2.value,y);
            if ( row == controlId && col == controlId )
                return alpha * if_( arg1.value, u ) * if_( arg2.value, u );
            return 0;
        }

        TrackingTypeCostFunctional( Scalar alpha_, ReferenceSolution const& fse,
                                    Direction const& d_, Scalar beta_ = 1.0 )
            : alpha( alpha_ ), beta( beta_ ), y_ref_fse( fse ), y_ref( 0 ), y( 0 ), u( 0 ), d( d_ )
        {
            assert( alpha > 0 );
        }
    ...
    public:
        ...
        {
            if ( row == stateId )
                return beta * ( y - y_ref ) * if_( arg.value, y );
            if ( row == controlId )
                return alpha * ( u * if_( arg.value, u ) );
            return 0;
        }
        ...
        {
            if ( row == stateId && col == stateId )
                return beta * if_( arg1.value, y ) * if_( arg2.value, y );
            if ( row == controlId && col == controlId )
                return alpha * if_( arg1.value, u ) * if_( arg2.value, u );
            return 0;
        }
        ...
        TrackingTypeCostFunctional( Scalar alpha_, ReferenceSolution const& fse,
                                    Scalar beta_ = 1.0 )
            : alpha( alpha_ ), beta( beta_ ), y_ref_fse( fse ), y_ref( 0 ), y( 0 ), u( 0 )
        {
            assert( alpha > 0 );
        }
