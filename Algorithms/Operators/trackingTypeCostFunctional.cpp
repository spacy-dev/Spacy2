...
namespace Algorithm
{
  ...
  double TrackingTypeCostFunctional::operator()(const AbstractFunctionSpaceElement& x) const
  {
    ...
    *stateDifference -= x_->primalVariable(stateId_);

    return 0.5 * (*stateDifference * *stateDifference)
        + 0.5 * alpha_ * ( x_->primalVariable(controlId_) * x_->primalVariable(controlId_) );
  }
  ...
  double TrackingTypeCostFunctional::d1(const AbstractFunctionSpaceElement& dx) const
  {
    ...
    *stateDifference -= x_->primalVariable(stateId_);
    ...
    return (*stateDifference * dx_.primalVariable(stateId_))
        + alpha_ * ( x_->primalVariable(controlId_) * dx_.primalVariable(controlId_) );
  }
  ...
  double TrackingTypeCostFunctional::d2(const AbstractFunctionSpaceElement& dx, const AbstractFunctionSpaceElement& dy) const
  {
    ...
    return (dx_.primalVariable(stateId_) * dy_.primalVariable(stateId_))
        + alpha_ * ( dx_.primalVariable(controlId_) * dy_.primalVariable(controlId_) );
  }
