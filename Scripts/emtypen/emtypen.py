...
def close_struct (comments):
    lines = data.form_lines

    expansion_lines = find_expansion_lines(lines)

    comment = get_comment('',comments,data.current_struct_prefix).rstrip('\n\r')
    ...
    for function in data.member_functions:
        nonvirtual_members += get_comment(indentation, comments, function[0])
...
def open_namespace (namespace_, comments = None):
    name = 'namespace ' + namespace_.spelling
    output[0] += '\n' + get_comment(indent(), comments, name)
    output[0] += indent() + name + ' {'
...
def visit_impl (cursor, parent, comments):
    ...
    if kind == clang.cindex.CursorKind.NAMESPACE:
        if from_main_file_:
            open_namespace(cursor, comments)
...
def visit (cursor, parent=None, comments=None):
    for child in cursor.get_children():
        result = visit_impl(child, cursor, comments)
        if result == child_visit.Recurse:
            if visit(child, cursor, comments) == child_visit.Break:
...
def add_headers( interface_headers ):
    output[0] += data.headers + '\n'
    for header in interface_headers:
        output[0] += header
    if len(interface_headers) > 0:
        output[0] += '\n'
...
include_guard = add_include_guard(data.filename)
add_headers(interface_headers)

visit(data.tu.cursor, None, comments)

if data.current_struct != null_cursor:
    close_struct(comments)

close_namespaces(data)
close_include_guard(include_guard)
