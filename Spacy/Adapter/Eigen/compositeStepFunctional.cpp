...
namespace Spacy
{
  namespace Rn
  {
    ...
    {
      const auto& x_ = cast_ref<Spacy::Rn::Vector>(Spacy::primalComponent(x)).get();
      return f_( x_ );
    }
    ...
    {
      const auto& x_ = cast_ref<Rn::Vector>(Spacy::primalComponent(x)).get();
      const auto& p = cast_ref<Rn::Vector>(Spacy::dualComponent(x)).get();
      
      auto m = x_.size(),  n = p.size();

      ::Eigen::VectorXd d_1(m+n);
      d_1 << df_(x_) + dc_(x_).transpose()*p, c_(x_);

      auto result = domain().zeroVector();

      ::Spacy::Rn::copy(d_1,result);
