...
namespace Spacy
{
  namespace CG
  {
    ...
    LinearSolver::LinearSolver(Operator A_, CallableOperator P_, MyOperator r_update, const std::string& type )
    : OperatorBase(A_.range(),A_.domain()),
      cg( std::move(A_), 
          std::move(P_), 
          std::move(r_update), 
  ...
  {
    auto solver = CG::LinearSolver(std::move(A), std::move(P), std::move(r_update), "ICG");
    solver.setRelativeAccuracy(relativeAccuracy);
    solver.set_eps(eps);
    solver.setVerbosity(verbose);
    return std::move(solver);
