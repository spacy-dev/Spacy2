...
namespace Spacy
{
    ...
    namespace CG
    {
        ...
        Solver::Solver( CallableOperator A, CallableOperator P, MyOperator r_update,
                        const std::string& type )
            : A_( std::move( A ) ), P_( std::move( P ) ),
              update_r_operator_( std::move( r_update ) ),
              terminate( CG::Termination::StrakosTichyEnergyError{} ), type_( type )
        {
            assert( type == "ICG" );
        }
        ...
        Vector Solver::cgLoop( Vector x, Vector r ) const
        {
            ...
            for ( unsigned step = 1; true; step++ )
            {
                ...
                // if( getVerbosityLevel() > 1 ) std::cout << "    " << type_ << "  sigma = " <<
                // sigma << ", alpha = "
        ...
        Vector Solver::icgLoop( Vector x, Vector r ) const
        {
            ...
            auto H_tilde_operator = [this]( const SV& loc_p, const SV& loc_w ) -> SV {
                SV loc_p1 = primal_projection(
                    loc_p );            // extract primal part st. there is no \tilde{C}^T*p_2
            ...
            SV w = zero( r.space() ), Hp = zero( r.space() );
            Real pHp = p( Hp );

            sigma = abs( r( Pr ) ); // preconditioned residual norm squared

            // the conjugate gradient iteration on inexact kernel
            for ( unsigned step = 1; step < 200; step++ )
            {
                w = get_w( r, Pr, beta, w );   // the heart of this algorithm
                Hp = H_tilde_operator( p, w ); // \tilde{H}*p_k
                pHp = p( Hp );

                alpha = -sigma / get( pHp ); // \alpha_k = \frac{- r_k^T p_k}{p_k^T \tilde{H} p_k}
                ...
                if ( vanishingStep( step ) )
                {
                    if ( step == 1 )
                        x += p;
                    result = Result::Converged;
                    std::cout << " !!pHp:\t" << pHp << " |r|\t" << norm( r ) << std::endl;
                    std::cout << "----------------------1" << std::endl;
                    break;
                }
                ...
                x += primal_projection( get( alpha ) * p );
                                              ...
                                              unsigned step ) const
        {
            ...
            if ( type_ == "CG" )
            {
                if ( verbose() )
                {
                    std::cout << "    " << type_ << ": Direction of negative curvature encountered "
                                                    "in standard CG Implementation!"
                              << std::endl;
                    std::cout << "    " << type_
                 ...
                 type_ == "ICG" )
            {
                ...
                if ( step == 1 )
                    x += q;
                if ( verbose() )
                    std::cout << "    " << type_ << ": Truncating at nonconvexity in iteration "
                              << step << ": " << qAq << std::endl;
            ...
            if ( type_ == "TRCG" || type_ == "RCG" )
            {
                ...
                if ( verbose() )
                    std::cout << "    " << type_ << ": Regularizing at nonconvexity in iteration "
                              << step << "." << std::endl;
                definiteness_ = DefiniteNess::Indefinite;
        ...
        void Solver::updateRegularization( Real qAq, Real qPq ) const
        {
            ...
            if ( getVerbosityLevel() > 1 )
                std::cout << "Updating regularization parameter from " << oldTheta << " to "
                          << theta << std::endl;
        }
        ...
        Vector Solver::get_w( const ::Spacy::Vector& r, const ::Spacy::Vector& g, const Real& beta,
                              ::Spacy::Vector w_old ) const
        {
            // w_{k-1} = \beta * w_{k-1}
            w_old *= get( beta );
