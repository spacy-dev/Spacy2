...
namespace Spacy
{
    ...
    namespace CompositeStep
    {
        ...
        void AffineCovariantSolver::tangentialSolverWasSet( bool val )
        {
            is_tangential_solver_set = val;
        }
                                                             ...
                                                             bool lastStepWasUndamped ) const
        {
            if ( is_tangential_solver_set )
            {
                ...
                {
                    tangentialSolver =
                        cast_ref< PPCG::LinearSolver >( L_.hessian( primalProjection( x ) ) ^ -1 );
                }
                else
                {
                    assert( false );
                                                     ...
                                                     bool lastStepWasUndamped ) const
        {
            ...
            //         auto trcg =
            //             makePPCGSolver( L_.hessian( x ), cgSolver.P(), get( trcgRelativeAccuracy
            //             ), eps(), verbose() );
            //         setParams( trcg );
            //         return IndefiniteLinearSolver( trcg );
            // }
            ...
            //    if( trcg == nullptr )
            auto trcg = makeTCGSolver( L_.hessian( x ), precond, get( trcgRelativeAccuracy ), eps(),
                                       verbose() );
            /*
            const auto& cgSolver = cast_ref<CG::LinearSolver>(normalSolver);
        ...
        Vector AffineCovariantSolver::computeMinimumNormCorrection( const Vector& x ) const
        {
            ...
            //    {
            //      const auto& P =
            //      cast_ref<CG::TriangularStateConstraintPreconditioner>(cgSolver.P());
            //      dn0 = P.kernelOffset(rhs);
            //      rhs -= cgSolver.A()( dn0 );
            //    }
            //  }
            if ( is< PPCG::LinearSolver >( normalSolver ) )
            {
                auto& cgSolver = cast_ref< PPCG::LinearSolver >( normalSolver );
                cgSolver.set_eps( eps() );
                cgSolver.setRelativeAccuracy( eps() );
                cgSolver.setVerbosity( verbose() );
                cgSolver.setVerbosityLevel( getVerbosityLevel() );
                cgSolver.setMaxSteps( 1e2 );
                dn0 = cgSolver.P()( rhs );
                rhs -= cgSolver.A()( dn0 );
            }
