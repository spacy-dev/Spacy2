...
namespace Spacy
{
    ...
    namespace CompositeStep
    {
        ...
        AffineCovariantSolver::AffineCovariantSolver(
            ...
            : retraction_( retraction ), dualUpdate_( linearRetraction ), N_( std::move( N ) ), L_( std::move( L ) ),
        ...
        AffineCovariantSolver::AffineCovariantSolver(
            ...
            : retraction_( retraction ), dualUpdate_( dualUpdate ), N_( std::move( N ) ), L_( std::move( L ) ),
        ...
        AffineCovariantSolver::AffineCovariantSolver( C2Functional N, C2Functional L, VectorSpace& domain )
            : retraction_( linearRetraction ), dualUpdate_( linearRetraction ), N_( std::move( N ) ),
                                                             ...
                                                             bool lastStepWasUndamped ) const
        {
            ...
            if ( is< PPCG::LinearSolver >( L_.hessian( primalProjection( x ) ) ^ -1 ) )
            {
                tangentialSolver = cast_ref< PPCG::LinearSolver >( L_.hessian( primalProjection( x ) ) ^ -1 );
            }
            else
            {
                assert( false );
            ...
            // */
            // /*
                // Jan Zetzmann: original implementation, which is incompatible with the ppcg -> TODO quick fix needs to be replaced by a proper solution. This may be handled by checking whether the setSolverCreator function was called
                if ( !L_ )
            ...
            // */
                ...
                std::cout << " tangential step rhs :"
                        << norm( primalProjection( -d1( L_, x ) ) + primalProjection( -nu * d2( L_, x )( dn ) ) )
                        << std::endl;
                std::cout << " = " << norm( primalProjection( -d1( L_, x ) ) ) << " + "
                        << norm( primalProjection( -nu * d2( L_, x )( dn ) ) ) << std::endl;
                return primalProjection(
                    tangentialSolver( primalProjection( -d1( L_, x ) ) + primalProjection( -nu * d2( L_, x )( dn ) ) ) );
                                                                            ...
                                                                            bool lastStepWasUndamped ) const
        {
            ...
            if ( nu == 1 && lastStepWasUndamped )
            {
                ...
                if ( norm_dx_old > 0 && lastStepWasUndamped )
                    trcgRelativeAccuracy =
                        min( max( getRelativeAccuracy() / norm_dx_old, trcgRelativeAccuracy ), getMinimalAccuracy() );
                if ( getVerbosityLevel() > 1 )
                {
                    std::cout << spacing2 << "relative accuracy = " << trcgRelativeAccuracy << std::endl;
                    std::cout << spacing2 << "absolute step length accuracy = " << getRelativeAccuracy() * norm( x )
                              << std::endl;
                }
            }
            ...
            if ( is< CG::LinearSolver >( normalSolver ) )
            {
                ...
                {
                    auto trcg =
                    makeTCGSolver( L_.hessian( x ), cgSolver.P(), get( trcgRelativeAccuracy ), eps(), verbose() );
                    setParams( trcg );
            ...
            // template <typename T>
            // if ( is< PPCG::InexactSolver<T> >( normalSolver ) )
            // { 
                
            //     const auto& cgSolver = cast_ref<
            //                                     PPCG::InexactSolver<T>( normalSolver );
            //         auto trcg =
            //             makePPCGSolver( L_.hessian( x ), cgSolver.P(), get( trcgRelativeAccuracy ), eps(), verbose() );
            //         setParams( trcg );
            //         return IndefiniteLinearSolver( trcg );
            // }


            auto precond = [](const Vector& v) {return v;};
            //    if( trcg == nullptr )
            auto trcg = makeTCGSolver( L_.hessian( x ), precond, get( trcgRelativeAccuracy ), eps(), verbose() );
            /*
            const auto& cgSolver = cast_ref<CG::LinearSolver>(normalSolver);

            std::cout << cgSolver.isPositiveDefinite() << std::endl;
            */
            ...
            return IndefiniteLinearSolver( trcg );
            /*
            return IndefiniteLinearSolver( cgSolver );
            */
        }
        ...
        AffineCovariantSolver::computeCompositeStep( DampingFactor& nu, Real norm_Dn, const Vector& x, const Vector& Dn,
                                                     const Vector& Dt )
        {
            ...
            do
            {
                ...
                //        std::cout << "|x| = " << norm(x) << ", |trial| = " << norm(trial) << ", norm(projected(trial))
                //        = " << norm(primalProjection(trial)) << std::endl;
        ...
        Real AffineCovariantSolver::updateOmegaL( const Vector& soc, Real q_tau, DampingFactor tau, Real norm_x,
                                                  Real norm_dx, const CompositeStep::CubicModel& cubic )
        {
            ...
            if ( getVerbosityLevel() > 1 )
            {
                ...
                //        std::cout << spacing2 << "cubic(tau): " << cubic(tau) << ", cubic(0): " << cubic(0) <<
                //        std::endl;
                //        std::cout << spacing2 << "L(primalProjection(soc)): " << L_(primalProjection(soc)) << ",
                //        |primalProjection(soc)| = " << norm(primalProjection(soc)) << std::endl;
                std::cout << spacing2 << "actual decrease: " << ( L_( primalProjection( soc ) ) - cubic( 0 ) )
                          << std::endl;
        ...
        DampingFactor
        AffineCovariantSolver::computeTangentialStepDampingFactor( Real norm_dn, Real norm_Dt,
                                                                   const CompositeStep::CubicModel& cubic ) const
        {
            ...
            if ( pow( getRelaxedDesiredContraction() / omegaC, 2 ) - norm_dn * norm_dn > 0 )
                maxTau = min( 1., sqrt( pow( 2 * getRelaxedDesiredContraction() / omegaC, 2 ) - norm_dn * norm_dn ) /
                                      norm_Dt );

            return DampingFactor( Scalar::findGlobalMinimizer( cubic, 0, maxTau, getDampingAccuracy() ) );
        }

        AffineCovariantSolver::AcceptanceTest AffineCovariantSolver::acceptedSteps( Real norm_x, Real norm_Dx,
                                                                                    Real eta )
        {
        ...
        void AffineCovariantSolver::regularityTest( DampingFactor nu, DampingFactor tau ) const
        {
            if ( !regularityTestPassed( nu ) )
                throw RegularityTestFailedException( "AffineCovariantSolver::regularityTest (nu,...)",
                                                     get( get( nu ) ) );
            //      if( !regularityTestPassed(tau) ) throw
            //      RegularityTestFailedException("AffineCovariantSolver::regularityTest (...,tau)",get(tau));
        }
    }
}
