...
//        std::cout << spacing2 << "L(primalProjection(soc)): " << L_(primalProjection(soc)) << ", |primalProjection(soc)| = " << norm(primalProjection(soc)) << std::endl;
    ...
    {
      if( !L_ ) return DampingFactor(1);
      if( norm_Dt < sqrtEps() ) return DampingFactor(1);
      ...
      if(maxTau >= 1e-12) tau = Scalar::findLogGlobalMinimizer( cubic,1e-8, maxTau , getDampingAccuracy() );
      if(tau <= 1e-8) tau = 0.0;
