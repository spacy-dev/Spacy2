...
namespace Spacy
{
    ...
    namespace CompositeStep
    {
        ...
        Vector AffineCovariantSolver::operator()(const Vector& x0)
        {
            ...
            for(decltype(getMaxSteps()) step = 1; step < getMaxSteps(); ++step)
            {
                ...
                if( getVerbosityLevel() > 1 )
                    std::cout << spacing2 << "|Dn|: " << norm(primalProjection(Dn)) <<  " nu = " << nu << std::endl;
                ...
                if( verbose() ) {
                    std::cout << spacing2 << "|Dt|: " << norm(Dt) << std::endl;
        ...
        IndefiniteLinearSolver AffineCovariantSolver::makeTangentialSolver(DampingFactor nu, const Vector &x, bool lastStepWasUndamped) const
        {
            ...
            auto setParams = [this,&x](auto& solver)
            {
                ...
                if( norm(primalProjection(x)) > 0)
                    solver.setAbsoluteAccuracy( max(eps(), getRelativeAccuracy()*norm(primalProjection(x))) );
                else
        ...
        Vector AffineCovariantSolver::computeMinimumNormCorrection(const Vector& x) const
        {
            ...
            if( is<CG::LinearSolver>(normalSolver) )
            {
                ...
                cgSolver.setIterativeRefinements(getIterativeRefinements());
        ...
        std::tuple<DampingFactor, Vector, Vector, Real, Real> AffineCovariantSolver::computeCompositeStep(DampingFactor & nu, Real norm_Dn,
        ...
        {
            if( !L_ || norm_Dt < sqrtEps() ) return DampingFactor(1);
            ...
            return DampingFactor(Scalar::findLogGlobalMinimizer( cubic, eps(), maxTau, getDampingAccuracy() ));
        }
