...
TEST(Kaskade,SingleSpace_CopyVector)
{
  ...
  ASSERT_DOUBLE_EQ( at_c<0>(Spacy::cast_ref<Spacy::Kaskade::Vector<Descriptions> >(w).get().data)[testIndex()] , testValue() );
...
TEST(Kaskade,ProductSpace_CopyVector_StateVariable)
{
  ...
  ASSERT_DOUBLE_EQ( norm(v).get() , testValue() );
