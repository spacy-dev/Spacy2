...
namespace VSA
{
  ...
  //  }
    ...
    ::VSA::Vector& Vector::variable(unsigned k)
    {
      if( isPrimalDual() )
      {
    ...
    const ::VSA::Vector& Vector::variable(unsigned k) const
    {
      if( isPrimalDual() )
      {
        if( creator().isPrimalSubSpaceId(k) )
    ...
    void Vector::setPrimalComponent(const ::VSA::Vector& y)
    {
      variables_[0] = y;
    }
    ...
    void Vector::setDualComponent(const ::VSA::Vector& y)
    {
      variables_[1] = y;
    }
